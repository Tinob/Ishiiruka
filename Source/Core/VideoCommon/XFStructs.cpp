...
inline void XFRegWritten(int transferSize, u32 baseAddress)
{
  ...
  {
    u32 newValue = g_VideoData.Peek<u32>(dataIndex * sizeof(u32));
...
void PreprocessIndexedXF(u32 val, int refarray)
{
  ...
  const u8* new_data = Memory::GetPointer(g_preprocess_cp_state.array_bases[refarray] + g_preprocess_cp_state.array_strides[refarray] * index);
