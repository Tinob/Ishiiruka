...
void WritePixelShaderCommonHeader(ShaderCode& out, API_TYPE ApiType, u32 num_texgens,
  bool per_pixel_lighting, bool bounding_box)
{
  ...
  GenerateVSOutputMembers(out, ApiType, per_pixel_lighting, num_texgens, "");
  out.Write("};\n");
}
...
PixelUberShaderUid GetPixelUberShaderUid(u32 components, const XFMemory &xfr, const BPMemory &bpm)
{
  ...
  uid_data.early_depth =
    bpm.UseEarlyDepthTest() &&
      (g_ActiveConfig.bFastDepthCalc || bpm.alpha_test.TestResult() == AlphaTest::UNDETERMINED) &&
...
void ClearUnusedPixelUberShaderUidBits(API_TYPE ApiType, PixelUberShaderUid* uid)
{
                          ...
                          const pixel_ubershader_uid_data& uid_data)
{
  ...
  if (ApiType == API_D3D11)
  {
    out.Write("// Helper function for Alpha Test\n"
              ...
              "}\n"
              "\n"
              "void setRegColor(inout State s, uint index, int3 col) {\n"
              "  switch (index) {\n"
              "  case 0u: // prev\n"
              "    s.Reg[0].rgb = col;\n"
              "    break;\n"
  ...
  else  // D3D
  {
    ...
    {
      for (u32 i = 0; i < numTexgen; ++i)
        out.Write(",\n  in %s float3 tex%d : TEXCOORD%d", optCentroid, i, i);
    ...
    else
    {
      ...
      {
        for (u32 i = 0; i < 8; ++i)
          out.Write(",\n  in %s float4 tex%d : TEXCOORD%d", optCentroid, i, i);
      }
      else
      {
        for (u32 i = 0; i < numTexgen; ++i)
          out.Write(",\n in %s float%d tex%d : TEXCOORD%d", optCentroid, i < 4 ? 4 : 3, i, i);
      }
    }
  ...
  if (per_pixel_lighting)
  {
    ...
    if (numTexgen < 7)
    {
      ...
      out.Write("float3 lit_pos = float3(clipPos.x,clipPos.y,Normal.w);\n");
    }
    else
    {
      out.Write("float3 lit_normal = normalize(float3(tex4.w,tex5.w,tex6.w));\n\n");
      out.Write("float3 lit_pos = float3(tex0.w,tex1.w,tex7.w);\n");
    }
  ...
  out.Write(
      ...
      "        coloro = tevLerp3(color_A, color_B, color_C, color_D, color_bias, color_op, false, "
      "color_shift);\n"
      "      } else { // Compare mode\n"
      "        // op 6 and 7 do a select per color channel\n"
      "        if (color_compare_op == 6u) {\n"
      "          // TEVCMP_RGB8_GT\n"
      "          coloro.r = (color_A.r > color_B.r) ? color_C.r : 0;\n"
      "          coloro.g = (color_A.g > color_B.g) ? color_C.g : 0;\n"
      "          coloro.b = (color_A.b > color_B.b) ? color_C.b : 0;\n"
      ...
      "          coloro = tevCompare(color_compare_op, color_A, color_B) ? color_C : int3(0, 0, "
      "0);\n"
      "        }\n"
      "        coloro = color_D + coloro;\n"
      "      }\n"
      "\n"
      "      // Clamp result\n"
      "      if (color_clamp)\n"
      "        coloro = clamp(coloro, 0, 255);\n"
      "      else\n"
      "        coloro = clamp(coloro, -1024, 1023);\n"
      "\n"
      "      // Write result to the correct input register of the next stage\n"
      "      setRegColor(s, color_dest, coloro);\n"
      "\n");
  ...
  if (ApiType == API_D3D11 && uid_data.uint_output)
  {
  ...
  out.Write("  if (ras < 2u) { // Lighting Channel 0 or 1\n"
            "    int4 col = iround(((ras == 0u) ? colors_0 : colors_1) * 255.0);\n"
            "    uint swap = %s;\n",
            BitfieldExtract("ss.ac", TevStageCombiner().alphaC.rswap).c_str());
  out.Write("    return Swizzle(swap, col);\n");
