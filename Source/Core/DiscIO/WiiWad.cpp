...
bool WiiWAD::ParseWAD(IBlobReader& reader)
{
	...
	offset += Common::AlignUpSizePow2(certificate_chain_size, 0x40);
	m_ticket = CreateWADEntry(reader, ticket_size, offset);
	offset += Common::AlignUpSizePow2(ticket_size, 0x40);
	m_tmd = CreateWADEntry(reader, tmd_size, offset);
	offset += Common::AlignUpSizePow2(tmd_size, 0x40);
	m_data_app = CreateWADEntry(reader, data_app_size, offset);
	offset += Common::AlignUpSizePow2(data_app_size, 0x40);
	m_footer = CreateWADEntry(reader, footer_size, offset);
	offset += Common::AlignUpSizePow2(footer_size, 0x40);
