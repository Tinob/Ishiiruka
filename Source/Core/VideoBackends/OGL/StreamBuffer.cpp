...
public:
	...
	{
		CreateFences();
		m_pointer = static_cast<u8*>(AllocateAlignedMemory(m_size, ALIGN_PINNED_MEMORY));
...
std::unique_ptr<StreamBuffer> StreamBuffer::Create(u32 type, u32 size)
{
	...
	if (g_ogl_config.bSupportsGLSync)
	{
		...
		if (g_ogl_config.bSupportsGLPinnedMemory &&
			!(DriverDetails::HasBug(DriverDetails::BUG_BROKEN_PINNED_MEMORY) && type == GL_ELEMENT_ARRAY_BUFFER))
