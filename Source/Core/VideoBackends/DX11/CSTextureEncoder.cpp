...
)HLSL"  // Visual do not like string literal of more than 16K and it is legit, that should be an
        // external resource
                                    R"HLSL(
// FIXME: Untested
...
void CSTextureEncoder::Init()
{
  ...
  auto outBd =
      CD3D11_BUFFER_DESC((4 * 4) * EFB_WIDTH * EFB_HEIGHT / 4, D3D11_BIND_UNORDERED_ACCESS);
  ...
  auto outUavDesc = CD3D11_UNORDERED_ACCESS_VIEW_DESC(m_out.get(), DXGI_FORMAT_R32_UINT, 0,
                                                      (4) * EFB_WIDTH * EFB_HEIGHT / 4);
  if (!bSupportsShaderModel5)
  {
  ...
  std::string cache_filename =
      StringFromFormat("%sdx11-ENCODER-cs.cache", File::GetUserPath(D_SHADERCACHE_IDX).c_str());
                              ...
                              bool is_depth_copy, const EFBRectangle& src_rect, bool scale_by_half)
{
  ...
  ID3D11ShaderResourceView* pEFB =
      is_depth_copy ? FramebufferManager::GetResolvedEFBDepthTexture()->GetSRV() :
  ...
  while ((hr = D3D::context->Map(m_outStage.get(), 0, D3D11_MAP_READ, D3D11_MAP_FLAG_DO_NOT_WAIT,
                                 &map)) != S_OK &&
...
static const char* FETCH_FUNC_NAMES[4] = {"Fetch_0", "Fetch_1"};

static const char* SCALEDFETCH_FUNC_NAMES[2] = {"ScaledFetch_0", "ScaledFetch_1"};

static const char* INTENSITY_FUNC_NAMES[2] = {"Intensity_0", "Intensity_1"};
...
ID3D11ComputeShader* CSTextureEncoder::GetEncodingComputeShader(const ComboKey& key)
{
  ...
  {
  case 0x0:
    generatorFuncName = "Generate_0";
    break;
  case 0x1:
    generatorFuncName = "Generate_1";
    break;
  case 0x2:
    generatorFuncName = "Generate_2";
    break;
  case 0x3:
    generatorFuncName = "Generate_3";
    break;
  case 0x4:
    generatorFuncName = "Generate_4";
    break;
  case 0x5:
    generatorFuncName = "Generate_5";
    break;
  case 0x6:
    generatorFuncName = "Generate_6";
    break;
  case 0x7:
    generatorFuncName = "Generate_7";
    break;
  case 0x8:
    generatorFuncName = "Generate_8";
    break;
  case 0x9:
    generatorFuncName = "Generate_9";
    break;
  case 0xA:
    generatorFuncName = "Generate_A";
    break;
  case 0xB:
    generatorFuncName = "Generate_B";
    break;
  case 0xC:
    generatorFuncName = "Generate_C";
    break;
  default:
  ...
  {
    WARN_LOG(VIDEO,
             "EFB encoder shader for dstFormat 0x%X, srcformat 0x%X, scaled : %d failed to compile",
