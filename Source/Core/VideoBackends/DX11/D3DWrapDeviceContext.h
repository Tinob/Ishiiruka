	...
	namespace D3D
	{
		...
		class WrapDeviceContext {
			...
			struct Cache {
				...
				std::array<ID3D11Buffer*, 8>             vsConstantBuffers_;
				std::array<bool, 8>                      vsConstantBufferDirties_;
				bool                                    vsConstantBufferDirty_;
				...
				Cache() {
					...
					vsConstantBuffers_.fill(nullptr);
					vsConstantBufferDirties_.fill(false);
				}
			};
		...
		public:
			...
			void VSSetConstantBuffers(UINT StartSlot, UINT NumBuffers, ID3D11Buffer *const *ppConstantBuffers){
				...
				bool dirty{};
				for (; StartSlot != NumBuffers; ++StartSlot) {
					if (c_.vsConstantBuffers_[StartSlot] != *ppConstantBuffers) {
						c_.vsConstantBuffers_[StartSlot] = *ppConstantBuffers;
						c_.vsConstantBufferDirties_[StartSlot] = true;
						dirty = true;
					}
					++ppConstantBuffers;
				}
				c_.vsConstantBufferDirty_ |= dirty;
			}
			...
			void GSSetConstantBuffers(UINT StartSlot, UINT NumBuffers, ID3D11Buffer *const *ppConstantBuffers) {
				if (StartSlot == 0) {
					if (c_.gsCb0_ != *ppConstantBuffers) {
						c_.gsCb0_ = *ppConstantBuffers;
					}
					else {
						++StartSlot;
						--NumBuffers;
		...
		private:
			void ApplyLazyStates() {
				...
				if (c_.psSrvDirty_) {
					for (u32 i = 0; i < c_.psSrvs_.size(); ++i) {
				...
				if (c_.samplerStateDirty_) {
					for (u32 i = 0; i < c_.samplerStates_.size(); ++i) {
				...
				if (c_.vsConstantBufferDirty_) {
					for (u32 i = 0; i < c_.vsConstantBuffers_.size(); ++i) {
						for (; i != c_.vsConstantBuffers_.size(); ++i) {
							if (c_.vsConstantBufferDirties_[i])
								break;
						}

						u32 j = i;
						for (; j != c_.vsConstantBuffers_.size(); ++j) {
							if (!c_.vsConstantBufferDirties_[j])
								break;
						}
						if (i != j) {
							ctx_->VSSetConstantBuffers(i, j - i, c_.vsConstantBuffers_.data() + i);
						}
						i = j;
					}
					c_.vsConstantBufferDirties_.fill(false);
					c_.vsConstantBufferDirty_ = false;
				}
			}
		};
